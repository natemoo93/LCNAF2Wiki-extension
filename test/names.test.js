...
test('keeps the period on an initial', () => {
  // In "Rothschild, Irwin B.," the period belongs to the initial.
...
test('flags an epithet rather than inverting it silently', () => {
  // The value is wrong but still returned, so it can be corrected in place.
...
test('a surname particle does not trigger the epithet check', () => {
  // "von" is in the surname, before the comma, so it proves nothing.
